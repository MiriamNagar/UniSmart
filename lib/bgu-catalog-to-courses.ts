 ...
 */
export function parseBguTimeField(time: string): { day: Days; startTime: string; endTime: string } | null {
	const t = time.trim();
	const full = /^יום\s*([אבגדהו])\s+(\d{1,2}:\d{2})\s*-\s*(\d{1,2}:\d{2})/.exec(t);
	if (full) {
...
function courseCreditsForGroup(rows: ParsedOffering[]): number {
	const fromLectures = rows.filter((r) => isLectureRow(r.type) && r.credits > 0);
