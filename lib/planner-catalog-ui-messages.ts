...
export function buildPlannerCatalogUiModel(input: {
	...
	const { hasFirebaseDb, source, loadError, loading, activeTermSummary } = input;

	if (loading) {
		return {
			showRetry: false,
			headline: 'Loading catalog…',
			bodyLines: [`Showing courses for your active term: ${activeTermSummary}.`],
	...
	if (!hasFirebaseDb) {
		return {
			...
			bodyLines: [
				'Firebase is not configured in this build, so the app uses the bundled sample catalog.',
	...
	return {
		showRetry: true,
		headline: 'Could not load the Firestore catalog',
		bodyLines: [
			loadError ? `Details: ${loadError}` : 'An unknown error occurred while loading.',
			'Showing the bundled sample catalog so you can keep planning.',
			`Courses are filtered for: ${activeTermSummary}.`,
			NON_ENROLLMENT,
		],
		compactNonEnrollmentNote: NON_ENROLLMENT,
