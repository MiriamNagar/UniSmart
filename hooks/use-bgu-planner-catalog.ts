...
export function useBguPlannerCatalog(): UseBguPlannerCatalogResult {
	...
	const refresh = useCallback(async () => {
		if (!db) {
			...
			setLoading(false);
			if (__DEV__) {
				console.log('[UniSmart] Planner catalog: bundled (Firebase db not initialized — check EXPO_PUBLIC_* in .env)');
			}
			return;
		}
		setLoading(true);
		...
		try {
			...
			if (__DEV__) {
				console.warn('[UniSmart] Planner catalog using bundled mock:', e);
			}
		} finally {
			setLoading(false);
		}
